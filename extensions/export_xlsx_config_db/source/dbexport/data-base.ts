...
export default class DataBase {
    ...
    generatePreviewDbJsonText() {
        ...
        let filePathMap = DbExporter.dbNames_2_dbFilePathMap[this.name];
        if (filePathMap) {
            let filePaths = [];
            Tools.forEachMap(filePathMap, (filePath) => {
                filePath = filePath.replaceAll(DbExporter.prjRootDir + "\\", "").replaceAll("\\", "/");
    ...
    generateAutoExportDbTsGettersText() {
        ...
        text += Tools.format("    db.get_%s = (): db.%s => { return DbManager.getDataBase('%s').datas; }\n", this.name, this.name, this.name);

        switch (this.rule) {
            case "m": {
                let majorField = this.fields[0];
                text += Tools.format("    db.get_from_%s = (%s: %s, bQuiet?: boolean): db.%s_data => { return DbManager.getDataBase('%s')._get1(%s, bQuiet); }\n",
                    this.name,
                    majorField.name,
                ...
                text += Tools.format("    db.foreach_from_%s = (callback: (%sKey: string, data: db.%s_data) => (void | boolean)) => { DbManager.getDataBase('%s')._foreachData1(callback); }\n",
                    this.name,
                    majorField.name,
                    this.name,
                    this.name,
            ...
            case "mm": {
                let majorField = this.fields[0];
                let minorField = this.fields[1];
                text += Tools.format("    db.get_from_%s = (%s: %s, %s: %s, bQuiet?: boolean): db.%s_data => { return DbManager.getDataBase('%s')._get2(%s, %s, bQuiet); }\n",
                    this.name,
                    majorField.name,
                    majorField.tsType == 'number' ? 'number | string' : majorField.tsType,
                    minorField.name,
                ...
                text += Tools.format("    db.foreach_from_%s = (callback: (%sKey: string, %sKey: string, data: db.%s_data) => (void | boolean)) => { DbManager.getDataBase('%s')._foreachData2(callback); }\n",
                    this.name,
                    majorField.name,
                    minorField.name,
                    this.name,
                ...
                text += Tools.format("    db.getMap_from_%s = (%s: %s, bQuiet?: boolean): { [%s: %s]: db.%s_data } => { return DbManager.getDataBase('%s')._get1(%s, bQuiet); }\n",
                    this.name,
                    majorField.name,
                    majorField.tsType == 'number' ? 'number | string' : majorField.tsType,
                    minorField.name,
                ...
                text += Tools.format("    db.foreachMap_from_%s = (callback: (%sKey: string, datas: { [%s: %s]: db.%s_data }) => (void | boolean)) => { DbManager.getDataBase('%s')._foreachData1(callback); }\n",
                    this.name,
                    majorField.name,
                    minorField.name,
                    minorField.tsType,
            ...
            case "ma": {
                let majorField = this.fields[0];
                text += Tools.format("    db.get_from_%s = (%s: %s, index: number, bQuiet?: boolean): db.%s_data => { return DbManager.getDataBase('%s')._get2(%s, index, bQuiet); }\n",
                    this.name,
                    majorField.name,
                    majorField.tsType == 'number' ? 'number | string' : majorField.tsType,
                    this.name,
                ...
                text += Tools.format("    db.foreach_from_%s = (callback: (%sKey: string, index: number, data: db.%s_data) => (void | boolean)) => { DbManager.getDataBase('%s')._foreachData2(callback); }\n",
                    this.name,
                    majorField.name,
                    this.name,
                    this.name,
                );
                text += Tools.format("    db.getArr_from_%s = (%s: %s, bQuiet?: boolean): db.%s_data[] => { return DbManager.getDataBase('%s')._get1(%s, bQuiet); }\n",
                    this.name,
                    majorField.name,
                    majorField.tsType == 'number' ? 'number | string' : majorField.tsType,
                    this.name,
                ...
                text += Tools.format("    db.foreachArr_from_%s = (callback: (%sKey: string, datas: db.%s_data[]) => (void | boolean)) => { DbManager.getDataBase('%s')._foreachData1(callback); }\n",
                    this.name,
                    majorField.name,
                    this.name,
                    this.name,
            ...
            case "a": {
                text += Tools.format("    db.get_from_%s = (index: number, bQuiet?: boolean): db.%s_data => { return DbManager.getDataBase('%s')._get1(index, bQuiet); }\n",
                    this.name,
                    this.name,
                    this.name,
                );
                text += Tools.format("    db.foreach_from_%s = (callback: (index: number, data: db.%s_data) => (void | boolean)) => { DbManager.getDataBase('%s')._foreachData1(callback); }\n",
                    this.name,
                    this.name,
                    this.name,
                );
    ...
    generateDbdtsGettersText() {
        ...
        text += Tools.format("    function get_%s(): db.%s;\n", this.name, this.name);

        switch (this.rule) {
            case "m": {
                let majorField = this.fields[0];
                text += Tools.format("    function get_from_%s(%s: %s, bQuiet?: boolean): db.%s_data;\n",
                    this.name,
                    majorField.name,
                    majorField.tsType == 'number' ? 'number | string' : majorField.tsType,
                    this.name,
                );
                text += Tools.format("    function foreach_from_%s(callback: (%sKey: string, data: db.%s_data) => (void | boolean)): void;\n",
                    this.name,
                    majorField.name,
                    this.name,
                );
            ...
            case "mm": {
                let majorField = this.fields[0];
                let minorField = this.fields[1];
                text += Tools.format("    function get_from_%s(%s: %s, %s: %s, bQuiet?: boolean): db.%s_data;\n",
                    this.name,
                    majorField.name,
                    majorField.tsType == 'number' ? 'number | string' : majorField.tsType,
                    minorField.name,
                ...
                text += Tools.format('    function foreach_from_%s(callback: (%sKey: string, %sKey: string, data: db.%s_data) => (void | boolean)): void;\n',
                    this.name,
                    majorField.name,
                    minorField.name,
                    this.name,
                );
                text += Tools.format('    function getMap_from_%s(%s: %s, bQuiet?: boolean): { [%s: %s]: db.%s_data };\n',
                    this.name,
                    majorField.name,
                    majorField.tsType == 'number' ? 'number | string' : majorField.tsType,
                    minorField.name,
                ...
                text += Tools.format('    function foreachMap_from_%s(callback: (%sKey: string, datas: { [%s: %s]: db.%s_data }) => (void | boolean)): void;\n',
                    this.name,
                    majorField.name,
                    minorField.name,
                    minorField.tsType,
            ...
            case "ma": {
                let majorField = this.fields[0];
                text += Tools.format('    function get_from_%s(%s: %s, index: number, bQuiet?: boolean): db.%s_data;\n',
                    this.name,
                    majorField.name,
                    majorField.tsType == 'number' ? 'number | string' : majorField.tsType,
                    this.name,
                );
                text += Tools.format('    function foreach_from_%s(callback: (%sKey: string, index: number, data: db.%s_data) => (void | boolean)): void;\n',
                    this.name,
                    majorField.name,
                    this.name,
                );
                text += Tools.format('    function getArr_from_%s(%s: %s, bQuiet?: boolean): db.%s_data[];\n',
                    this.name,
                    majorField.name,
                    majorField.tsType == 'number' ? 'number | string' : majorField.tsType,
                    this.name,
                );
                text += Tools.format('    function foreachArr_from_%s(callback: (%sKey: string, datas: db.%s_data[]) => (void | boolean)): void;\n',
                    this.name,
                    majorField.name,
                    this.name,
                );
            ...
            case "a": {
                text += Tools.format('    function get_from_%s(index: number, bQuiet?: boolean): db.%s_data;\n',
                    this.name,
                    this.name,
                );
                text += Tools.format('    function foreach_from_%s(callback: (index: number, data: db.%s_data) => (void | boolean)): void;\n',
                    this.name,
                    this.name,
                );
                break;
