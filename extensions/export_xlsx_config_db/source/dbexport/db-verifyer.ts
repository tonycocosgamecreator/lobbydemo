...
export class DbVerifyer {
    /**
     * { [dbName: string]: boolean}
     */
    static dbName_2_idExists;

    static aluId_2_exists;
    ...
    static verifyDb() {
        ...
        // 1. 打印配置表自身的loading警告
        Tools.forEachMap(DbExporter.dbName_2_db, (dbName, db) => {
        ...
        this.url_2_exists = {};
        let resourcesRootDir = path.join(DbExporter.prjRootDir, "assets", "resources");
        // console.log('resourcesRootDir', resourcesRootDir)

        Tools.foreachDir(resourcesRootDir, (filePath) => {
        ...
        Tools.forEachMap(DbExporter.dbName_2_db, (dbName, db) => {
            for (let i = 0; i < db.fields.length; i++) {
                ...
                if (field.idMergeTo) {
                    ...
                    let targetDb = DbExporter.dbName_2_db[field.idMergeTo];
                    if (!targetDb) {
        ...
        // 5. 收集alu信息
        let alu_db = DbExporter.dbName_2_db["origin_alu_db"];
        this.aluId_2_exists = {};
        if (alu_db) {
            // console.log(alu_db)
