...
@ccclass('CustomLuckRankUser')
export default class CustomLuckRankUser extends ViewBase {
    ...
    init(index: number, data: game.SUDSevenUpDownRankList | null) {
        this._index = index;
    ...
    updateUserHead(data: game.SUDSevenUpDownRankList | null) {
        if (!data) {
            this._playerId = '-1';
            this.node.active = false;
            return;
    ...
    updatePlayBalance() {
        ...
        const data = CommonManager.getLuckPlayerDataById(this._playerId);
        if (!data) return;
        this.labelCoin.string = CurrencyHelper.format(data.balance, this._currency, { showSymbol: true });
    }
     ...
     * 请勿修改，脚本自动生成
    */
    public static readonly VIEW_NAME = 'CustomLuckRankUser';
    /**
     * 当前界面的所属的bundle名字
     * 请勿修改，脚本自动生成
    */
    ...
    */
    public get bundleName() {
        return CustomLuckRankUser.BUNDLE_NAME;
    }
    public get viewName() {
        return CustomLuckRankUser.VIEW_NAME;
    }
