...
@ccclass('CustomChipAnimation')
export default class CustomChipAnimation extends ViewBase {
    ...
    initData(stage: number) {
        this.reset();
        if (stage == baccarat.DeskStage.StartBetStage || baccarat.DeskStage.EndBetStage || baccarat.DeskStage.OpenStage) {
            this.reconnectChip();
        }
    }
    ...
    updateflyChip(data: betInfo, order: number) {
        ...
        chip.node.scale = order == -1 ? v3(0, 0, 0) : v3(this._middle.x * 1.1, this._middle.y * 1.1, 1);
        const startWorldPos = order == -1 ? GameManager.View.getWorldPosByUid(data.player_id) : v3(targetWorldPos.x, targetWorldPos.y + 30, targetWorldPos.z);
        ...
        if (order == -1) {
            this.flyToTarget(chip.node, endPos, true, call);
        } else {
            this.dropToTarget(chip.node, endPos, call);
        }
    }
    ...
    reconnectChip() {
        const list = GameManager.getBetInfoByPlay();
    ...
    reset() {
        for (let i = 0; i < 14; i++) {
            this._count[i] = i == 6 || i == 12 || i == 13 ? this.getRandomInRange(25, 30) : this.getRandomInRange(5, 8);
        }
    ...
    _addFlyAnimation(tweenChain: cc.Tween<cc.Node>, flyDuration: number, endPos: Vec3) {
        const scale = new Vec3(this._middle.x * 1.1, this._middle.y * 1.1, 1)
    ...
    dropToTarget(flyObject: cc.Node, endPos: cc.Vec3, callback: () => void = null, dur: number = 0.2) {
        const opacity = flyObject.getComponent(UIOpacity);
        Tween.stopAllByTarget(opacity);
        opacity.opacity = 255;
        tween(flyObject)
     ...
     * 请勿修改，脚本自动生成
    */
    public static readonly VIEW_NAME = 'CustomChipAnimation';
    /**
     * 当前界面的所属的bundle名字
     * 请勿修改，脚本自动生成
    */
    ...
    */
    public get bundleName() {
        return CustomChipAnimation.BUNDLE_NAME;
    }
    public get viewName() {
        return CustomChipAnimation.VIEW_NAME;
    }
