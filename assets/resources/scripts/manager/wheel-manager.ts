...
export default class WheelManager extends BaseManager {
    //=============================子类需要自己实现的方法===========================//
    /**
     * 存档的KEY,也是管理器的key
     */
    public static KEY = 'WheelManager';
    /**
     * 你属于哪个bundle
     */
    ...
    public static onNetMessage(msgType: string, data: any): boolean {
        switch (msgType) {
            ...
            case wheel.Message.MsgPlayerRankNtf: {
                const msg = data as wheel.MsgPlayerRankNtf;
            ...
            case wheel.Message.MsgBetBaccaratNtf: {
                const msg = data as wheel.MsgBetBaccaratNtf;
                if (msg.desk_id != this._deskId) return false;
            ...
            case wheel.Message.MsgCancelBetSevenUpDownRsp: {
                const msg = data as wheel.MsgCancelBetSevenUpDownRsp;
                const result = msg.result;
                if (result && result.err_code != commonrummy.RummyErrCode.EC_SUCCESS) {
                    //如果有错误码，说明下注失败了
                    //这里可以弹出提示框，提示玩家下注失败
            ...
            case wheel.Message.MsgCancelBetBaccaratNtf: {
                const msg = data as wheel.MsgCancelBetBaccaratNtf;
                if (msg.desk_id != this._deskId) return false;
                const plays = msg.players || [];
                for (let i = 0; i < plays.length; i++) {
                    const play = plays[i];
     ...
     * 当前所有的开奖记录
     */
    private static _records: wheel.SUDSevenUpDownWinType[] | null = null;
    /**
     * 前三名玩家数据
     */
    private static _topPlayers: Map<string, wheel.RankList> = new Map();
    /**
     * 当局玩家下注记录分数//记录排行榜用的
     */
    ...
    */
     ...
     * @param ranks 
     */
    public static setTopPlayerData(ranks: wheel.RankList[]) {
     ...
     * @param ranks 
     */
    public static getTopPlayerData(): wheel.RankList[] {
     ...
     * @param playerId 
     */
    public static getTopPlayerDataById(playerId: string): wheel.RankList | null {
